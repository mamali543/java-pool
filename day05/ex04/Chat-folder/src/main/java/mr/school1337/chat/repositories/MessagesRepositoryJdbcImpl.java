...
public class MessagesRepositoryJdbcImpl implements MessagesRepository{
    ...
    public void update(Message msg){
        String sqlQuery = "UPDATE chat.message SET author_id = ? , room_id = ? ,content = ?, datetime = ? WHERE id = ? RETURNING content;";
        try (Connection connection = dataSource.getConnection();
        PreparedStatement ps = connection.prepareStatement(sqlQuery)){
            ...
            if (msg.getRoom() != null && msg.getRoom().getId() != null)
                ps.setLong(2, msg.getRoom().getId());
            else
            ...
            try(ResultSet rs = ps.executeQuery()){
                if (rs.next()){
                    String message_content = rs.getString(1);
                    System.out.println(ANSI_BLUE+"New message content: " + message_content +ANSI_RESET);
                }
            }
        }
        catch(SQLException e)
